...
function ReelItem({
  ...
  // Sync muted prop → video element (React controlled attribute doesn't update DOM)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Full-screen tap to toggle mute — behind interactive elements */}
      <button
      ...
      <div style={{
        ...
        <button
          onClick={handleLike}
          style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, background: 'none', border: 'none', cursor: currentUserId ? 'pointer' : 'default' }}
        >
          <svg width="28" height="28" viewBox="0 0 24 24"
            fill={liked ? '#FF4D4D' : 'none'}
            stroke={liked ? '#FF4D4D' : 'white'}
      ...
      <div style={{ position: 'absolute', bottom: 74, left: 14, right: 72, zIndex: 4 }}>
        ...
        {reel.author && (
          <Link
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 8, position: 'relative' }}
          >
            ...
            <span style={{ color: 'white', fontSize: 14, fontWeight: 600, textShadow: '0 1px 3px rgba(0,0,0,0.4)' }}>
        ...
        {/* Caption */}
        {reel.body && (
          <p style={{ color: 'white', fontSize: 14, lineHeight: 1.45, margin: 0, textShadow: '0 1px 3px rgba(0,0,0,0.35)' }}>
            {reel.body}
          </p>
        )}
      </div>
...
export default function ReelsPage() {
  ...
  if (loading) {
    return (
      <>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 40, background: '#0F2240', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  if (reels.length === 0) {
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 40, background: '#0F2240', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
