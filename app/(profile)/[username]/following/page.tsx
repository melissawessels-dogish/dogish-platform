...
const TABS = ['people', 'dogs', 'breeds'] as const
...
export default async function FollowingPage({
  ...
  const { tab: tabParam = 'people' } = await searchParams
  ...
  // Fetch all three in parallel — counts are needed for tab labels regardless of active tab
  const [peopleRes, dogsRes, breedsRes] = await Promise.all([
    ...
    admin
      .from('follow')
      .select('id, breed:breed!target_breed_id(id, name, follower_count)')
      .eq('follower_id', profileHuman.id)
  ...
  return (
    <div className="min-h-svh bg-white">
      <div className="max-w-[640px] mx-auto">
        ...
        {/* Header */}
        <div className="px-4 pt-3 pb-0 sticky top-0 bg-white z-10 border-b border-[#0F2240]/8">
          ...
          <div className="flex gap-6">
            {TABS.map((tab) => (
              <Link
                key={tab}
                href={`/${username}/following?tab=${tab}`}
              ...
              >
                ...
                {counts[tab] > 0 && (
                  <span className="ml-1 text-[12px]" style={{ color: activeTab === tab ? 'rgba(15,34,64,0.5)' : 'rgba(15,34,64,0.3)' }}>
                    ({counts[tab]})
                  </span>
                )}
                {activeTab === tab && (
        ...
        <div className="px-4 pb-24">
          ...
          {activeTab === 'dogs' && (
            dogs.length === 0 ? (
              <p className="text-center text-[14px] text-[#0F2240]/40 py-14">Not following any dogs yet.</p>
            ) : (
          ...
          {activeTab === 'breeds' && (
            breeds.length === 0 ? (
              <p className="text-center text-[14px] text-[#0F2240]/40 py-14">Not following any breeds yet.</p>
            ) : (
                ...
                <div key={follow_id} className="flex items-center justify-between gap-3 py-3 border-b border-[#0F2240]/6 last:border-0">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {breed.follower_count != null && (
                        <p className="text-[11px] text-[#0F2240]/45">{breed.follower_count.toLocaleString()} followers</p>
                      )}
                    </div>
                  </div>
